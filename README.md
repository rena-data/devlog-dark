...
| 버전 | 날짜 | 주요 변경사항 |
|------|------|-------------|
| v1.1 | 2026-06-03 | 방명록 다크모드 수정, canonical URL 추가, 모바일 이전/다음글 오버플로 수정, 분류 전체보기 → 전체 글 텍스트 변경, 스크롤 버튼 위치 조정, 메뉴바/구독 위치 안내, 미사용 코드 삭제 |
| v1.1 | 2026-05-21 | 반응형 5단계 breakpoint, SEO(Schema.org JSON-LD), 성능 최적화(font-display swap, defer, passive, preload), 구독 네임카드 일원화, 코드블럭 가시성 강화, 다크모드 인용/테이블/광고 오버플로 수정, 스크롤 버튼 항상 표시, 불필요 코드 정리 |
